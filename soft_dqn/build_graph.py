...
def build_train(make_obs_ph, q_func, num_actions, optimizer, grad_norm_clipping=None, gamma=1.0,
    double_q=True, scope="deepq", reuse=None, param_noise=False, param_noise_filter_func=None, soft_max=True, Temperature=1.0):
...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if soft_max:
            q_t_selected_target = rew_t_ph + gamma * tf.reduce_logsumexp(Temperature* q_tp1, 1) / Temperature
        else:
        ...
        if grad_norm_clipping is not None:
            gradients_q = optimizer.compute_gradients(
                weighted_error, var_list=q_func_vars)
            for i, (grad, var) in enumerate(gradients_q):
                if grad is not None:
                    gradients_q[i] = (tf.clip_by_norm(
                        grad, grad_norm_clipping), var)
            optimize_expr = optimizer.apply_gradients(gradients_q)
